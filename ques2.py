...
def input_graph(G):
    ...
    G.add_edge("2", "3", weight=4, capacity=10)
    G.add_edge("2", "4", weight=3, capacity=10)
    G.add_edge("3", "4", weight=5, capacity=5)
    G.add_edge("3", "5", weight=5, capacity=10)
    G.add_edge("5", "4", weight=2, capacity=15)
...
nx.draw_networkx_edge_labels(G, pos=position, edge_labels=nx.get_edge_attributes(G, 'capacity'))
flowCost, flowDict = nx.capacity_scaling(G)

# Print result
printPath(flowDict, G)

mpl.savefig("Graph.png")
mpl.show()